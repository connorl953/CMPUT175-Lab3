...
def getEntry(player, entries):
    ...
    while True:
        try:
            ...
            if int(entry) < lowerRange or int(entry) > upperRange:
                raise ValueError
            if int(entry) in entries:
                raise ValueError
            ...
            print('Invalid input. Please enter an {} number ({}-{}) that has not been played yet.'.format(numDescription, lowerRange, upperRange))
...
def getCoord(player, dimension):
    ...
    while True:
        try:
            ...
            if int(index) < LOWER or int(index) > UPPER:
                raise ValueError
            return int(index)
        except ValueError:
